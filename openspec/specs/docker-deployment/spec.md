...
The script SHALL validate registry authentication before attempting to push.
...
#### Scenario: Registry authentication required

- **WHEN** user is not authenticated to the Forgejo registry
- **THEN** script SHALL fail with message instructing to run `docker login forgejo.home.janbaer.de`
