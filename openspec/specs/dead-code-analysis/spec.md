...
This capability codifies how the project uses `fallow` to detect and prevent dead code. It defines the `.fallowrc.json` configuration required for fallow to correctly understand the project's entry-point set (Bun test runner, orchestration scripts, transitive meta-package dependencies) and sets the policy that the repository must maintain a zero-finding baseline so future dead code is visible the moment it appears.
