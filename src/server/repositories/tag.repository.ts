...
export class TagRepository extends BaseRepository<Tag> {
  ...
  findByNameAndUserId(name: string, userId: string): Tag | null {
    return db
      .query<Tag, [string, string]>(`SELECT * FROM tags WHERE name = ? COLLATE NOCASE AND user_id = ?`)
      .get(name, userId);
  }
  ...
  getItemCountForTag(tagId: string): number {
    const result = db
      .query<{ count: number }, [string]>(`SELECT COUNT(*) as count FROM item_tags WHERE tag_id = ?`)
      .get(tagId);
  ...
  setItemTags(itemId: string, tagIds: string[]): void {
    ...
    for (const tagId of tagIds) {
      db.run(`INSERT INTO item_tags (item_id, tag_id) VALUES (?, ?)`, [itemId, tagId]);
    }
  }
  ...
  findAllByUserId(userId: string): Tag[] {
    return db.query<Tag, [string]>(`SELECT * FROM tags WHERE user_id = ? ORDER BY name COLLATE NOCASE ASC`).all(userId);
  }
