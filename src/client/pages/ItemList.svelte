    ...
    {:else}
      <div class="items-masonry">
        {#each itemList as item, i}
          <article
            class="item-card group flex flex-col rounded-xl border border-border bg-card p-4 shadow-sm transition-shadow hover:shadow-md fade-in relative"
          ...
          >
            ...
            <!-- Footer: tag + date (pushed to bottom) -->
...
<style>
  ...
  .items-masonry {
    columns: 32rem; /* 480px column width */
    column-gap: 1rem;
  }
  ...
  /* Answer preview with markdown rendering */
  .answer-preview {
    max-height: 15rem;
    overflow: hidden;
    position: relative;
